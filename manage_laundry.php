...
<div class="container-fluid">
    ...
    <label for="inventory_item_id">Select Inventory Item:</label>
    <select id="inventory_item_id" name="inventory_item_id" required>
        <?php 
        // Query to get inventory items from both tables
        $query = "
            SELECT i.id, s.name, i.qty 
            FROM inventory i
            JOIN supply_list s ON i.id = s.id
        ";
        $result = $conn->query($query);
        while ($row = $result->fetch_assoc()) { 
        ?>
            <option value="<?php echo $row['id']; ?>" data-qty="<?php echo $row['qty']; ?>">
                <?php echo $row['name'] . " (Qty: " . $row['qty'] . " available)"; ?>
            </option>
        <?php } ?>
    </select>
...
$('#inventory_item_id').change(function() {
    ...
    var selected_qty = $(this).find('option:selected').attr('data-qty'); // Get the available quantity of the selected inventory item

    // Display the available quantity in a specific element (e.g., a span or input field)
    $('#inventory_qty_display').text("Available quantity: " + selected_qty);
});
$('#add_to_list').click(function(){
    var cat = $('#laundry_category_id').val(),
        _weight = $('#weight').val(),
        inventory_id = $('#inventory_item_id').val();

    if(cat == '' || _weight == '' || inventory_id == ''){
        alert_toast('Fill the category, weight, and inventory item fields first.','warning');
        return false;
    }
    ...
    var inventory_qty = $('#inventory_item_id option[value="'+inventory_id+'"]').attr('data-qty');

    // Check if there's enough inventory
    if (parseInt(inventory_qty) <= 0) {
        alert_toast('Insufficient inventory quantity.','warning');
        return false;
    }

    // Add item to the list
    var tr = $('<tr></tr>');
    tr.attr('data-id', inventory_id);
    ...
    var new_qty = parseInt(inventory_qty) - 1;
    $('#inventory_item_id option[value="'+inventory_id+'"]').attr('data-qty', new_qty);

    // Update the displayed available quantity in the dropdown
    $('#inventory_item_id option[value="'+inventory_id+'"]').text(function(index, text) {
        return text.replace(/\(\d+ available\)/, '(' + new_qty + ' available)');
    });
